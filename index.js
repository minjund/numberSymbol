module.exports = function (data, afterDotLength, isSymbol= false) {
    // data type -> String
    ...
    */
    let result = arr[1] ? arr[0].replace(regexp, ',') + '.' + arr[1] : arr[0].replace(regexp, ',')
    if (isSymbol) {
        ...
        arrayReslut.forEach(() => {
            if (!isNaN(afterDotLength) && arrayReslut.length > 1) {
                ...
                //   result = arr[0].replace(regexp, ',')
                // }
                // 자릿수 초과하면 NaN 처리 필요
                result = Number(arrayReslut[0] + '.' + data.substring(arrayReslut[0].length, arrayReslut[0].length + afterDotLength)) + symbol
            } else {
